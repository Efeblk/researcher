...
Canonical work identity gives every valid DOI global priority. Explicit relations retained in an observation's provider payload can override that default: anchored Crossref `is-version-of`/`has-version` evidence uses one unambiguous, acyclic parent DOI, while anchored ORCID `version-of` evidence creates a deterministic source identity without claiming that either DOI is a concept DOI. Other relation types never merge works. These rules preserve each observation's DOI and provider payload. Durable canonical DOI aliases and relations make later researcher syncs reject contradictory parents and cycles. If related DOI aliases already belong to multiple canonical rows, reconciliation preserves those owners instead of coalescing them automatically. A provider must actually return the relation, so a DOI pair is not guaranteed to merge merely because a registry outside the collected payload relates it. Provider categories also remain unchanged unless the collected provider supplied the classification.

DOI-less records first use strict compatible-author matching, then a final fallback merges them within the same `PersonelID` when normalized exact titles and known years agree. The fallback accepts missing, different, or truncated author text. With exactly one resolved DOI identity in the title/year bucket, DOI-less records attach to it. With multiple DOI identities, the fallback does not bridge them and preserves any stronger assignments. A bucket without a DOI uses a deterministic researcher/title/year identity. After identity-rule changes, reset the disposable development database and recollect; no legacy backfill is maintained.
