...
Every successful V1 `Collect` reconciles canonical links after normalized `AcademicWork` persistence and before `PublicationSummary` synchronization. Durable bulk jobs call that same collection service, so `Bulk/Submit` records are canonicalized when the default-enabled bulk worker processes them. YÖKSİS and Crossref refresh paths use the same sequence. The same local transaction schedules automatic article analysis from the resulting canonical observations; Semantic Scholar schedules again atomically when it later adds a full-text source. Reconciliation removes stale observations and memberships only for the requested researcher, keeps other researchers' associations, and never deletes orphaned canonical identities.
...
Successful manual or automatic article summaries attach normalized source snapshots and model-checked claim evidence to the canonical work. The save rechecks the work identity, deterministic source input, current association, and automatic execution token under the canonical write gate after the model call, so a refresh cannot attach stale input to a different publication or queue generation. Evidence remains historical if a provider observation is later replaced or removed, while canonical reads still require a current association. Collection only schedules the durable job; the dedicated worker performs article acquisition and analysis after the collection transaction commits. See [Article summaries](ARTICLE_SUMMARIES.md).
