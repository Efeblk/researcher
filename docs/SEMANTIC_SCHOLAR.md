...
`HasPendingWork`, bu çalıştırmanın güvenli sınır içinde tamamlanamadığını bildirir. Özellikle istek kotası veya sayfa ortası 429 yanıtından sonra açık toplama ucu tekrar çağrılmalıdır. `CitationsComplete=false` tek başına bekleyen iş anlamına gelmez: `CitationsFetched`, ayarlanmış `MaximumCitationsPerPaper` üst sınırına ulaştıysa sonuç bilinçli olarak kırpılmıştır ve sonraki çağrı aynı sınırla daha fazla atıf çekmez.
...
| `SemanticScholar:MaximumCitationsPerPaper` | 500 | Makale başına saklanacak en fazla atıf; bilinçli ve ayarlanabilir üst sınır. |
...
Toplu toplamada yerel hız sınırlayıcının ertelemesi, işi başarısız saymak yerine sonraki kuyruk denemesine bırakır. Atıf sayfası ortasında 429 veya geçici aktarım hatası oluşursa sonraki çağrı kaydedilmiş ofsetten sürer. Yenileme sırasında daha önce tamamlanmış atıf satırları silinmez; yeni nesil tamamlanınca atomik olarak değiştirilir. `CitationsRefreshing` yenilemenin sürdüğünü, `CitationsFetched` mevcut yenileme neslinde alınan sayıyı, `StoredCitationCount` ise o anda saklanan toplam ilişki sayısını gösterir.
