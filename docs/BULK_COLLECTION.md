...
Her satır zorunlu, benzersiz `PersonelID` ile isteğe bağlı `ORCID`, `ResearcherID` (Web of Science), `ScholarID` ve `ScopusID` alanlarını taşır. En az bir desteklenen kimlik gerekir. ScopusID 5–20 ASCII rakamdan oluşur; Scopus yazar bağlantısı da kanonik kimliğe çevrilebilir. YÖKSİS toplu akışta yoktur.
