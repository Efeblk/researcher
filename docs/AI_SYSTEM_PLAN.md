...
The opt-in evaluation workflow compares three named, versioned profiles without changing the production AI provider or model. Every run includes an immutable bilingual synthetic calibration pack with 18 controlled source-reading claims balanced across `supported`, `unsupported`, and `uncertain`; expected answers remain in the collector and are never sent to the model. A run may also copy up to three already-saved canonical sources for real-article review and ask later profiles to blind-cross-check the first profile's findings.
