...
`ResearcherAnalysisService/` owns all analysis products: researcher and article analysis, summaries, reviews, metrics, knowledge/graph, evaluations, HR dossiers, faculty workflows, their persistence, APIs, and workers. Diagnostic and profile-discovery controllers remain under `Api/V1`; product controllers and workflows live under `Products/`; provider adapters under `Integrations/`; private collector-source mappings under `SourceData/`; and analysis workers under `Background/`. Shared engine contracts live in `ResearcherAnalysis.Contracts/`. Collector HTTP examples live in `Requests/AcademicCollector/`; analysis examples live in `ResearcherAnalysisService/Requests/`; `Requests/README.md` is the surface index.
