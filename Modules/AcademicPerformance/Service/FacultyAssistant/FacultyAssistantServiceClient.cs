...
public sealed class FacultyAssistantServiceClient(HttpClient client, IOptions<AnalysisServiceOptions> options)
{
    ...
    {
        using HttpRequestMessage message = new(HttpMethod.Post, "api/v1/faculty-assistant")
        { Content = JsonContent.Create(request, options: JsonOptions) };
        if (!string.IsNullOrWhiteSpace(options.Value.ApiKey))
            message.Headers.Add("X-Analysis-Key", options.Value.ApiKey);
        using HttpResponseMessage response = await client.SendAsync(message, cancellationToken);
        if (!response.IsSuccessStatusCode)
            throw new HttpRequestException("Faculty assistant analysis failed.", null, response.StatusCode);
        FacultyAssistantAnalysisReport? report = await response.Content.ReadFromJsonAsync<FacultyAssistantAnalysisReport>(
