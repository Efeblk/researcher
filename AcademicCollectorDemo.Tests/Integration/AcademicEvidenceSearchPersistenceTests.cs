...
public sealed class AcademicEvidenceSearchPersistenceTests(SqlServerFixture fixture)
{
    [Fact]
    public async Task SearchAsync_LabelledFixture_PreservesIdentityOwnershipAndRetrievalDenominators()
    {
        ...
        database.CanonicalArticleAnalysisRuns.Add(new()
        {
            CanonicalWork = unassociated, ArticleSourceSnapshot = unassociatedSource,
    ...
    private static async Task<BridgeFixture> SeedBridgeFixtureAsync(AcademicDbContext database)
    {
        ...
        static CanonicalArticleAnalysisRun Run(int workId, long snapshotId,
            SavedArticleSummary summary, string language, string verificationStatus) => new()
        {
