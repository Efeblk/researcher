...
public sealed class ServiceAcceptanceBudget
{
    ...
    public const int MaximumCalls = 32;
    public const decimal MaximumSpendUsd = 1.00m;
    ...
    public ServiceAcceptanceBudget(Func<ServiceAcceptanceBudgetSnapshot, Task> persist,
        ServiceAcceptanceBudgetSnapshot? restored = null)
    {
        ...
        if (restored is not null) ValidateRestored(restored);
    }
        ...
        try
        {
            ...
            lock (gate)
            {
                ...
                if (calls.Count >= MaximumCalls)
                    throw new ServiceAcceptanceBudgetException("The aggregate 32-call ceiling was reached.");
                if (committedSpendUsd + reserved > MaximumSpendUsd)
                    throw new ServiceAcceptanceBudgetException("The next reservation would exceed aggregate USD 1.00.");
                committedSpendUsd += reserved;
                calls.Add(new(calls.Count + 1, startedAt, model, bodyBytes, maximumOutputTokens,
    ...
    private static void ValidateRestored(ServiceAcceptanceBudgetSnapshot value)
    {
        ...
        if (value.MaximumCalls != MaximumCalls || value.MaximumSpendUsd != MaximumSpendUsd ||
            ...
            value.Items.Any(item => item.RequestedModel != RequiredModel || item.BodyBytes <= 0 ||
...
public sealed record ServiceAcceptanceCall(int Ordinal, DateTime StartedAt, string RequestedModel,
...
internal sealed class AtomicBudgetStore(string path)
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web) { WriteIndented = true };
    public async Task WriteAsync(ServiceAcceptanceBudgetSnapshot value)
    {
        Directory.CreateDirectory(Path.GetDirectoryName(path)!);
        string temporary = path + ".pending";
        await File.WriteAllTextAsync(temporary, JsonSerializer.Serialize(value, JsonOptions), Encoding.UTF8);
        File.Move(temporary, path, true);
    }

    public async Task<ServiceAcceptanceBudgetSnapshot?> ReadAsync() => !File.Exists(path) ? null :
        JsonSerializer.Deserialize<ServiceAcceptanceBudgetSnapshot>(await File.ReadAllTextAsync(path), JsonOptions)
        ?? throw new InvalidOperationException("The existing acceptance ledger is unreadable and cannot be reset.");
}
