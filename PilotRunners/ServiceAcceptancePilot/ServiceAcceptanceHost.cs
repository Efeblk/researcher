...
internal static class ServiceAcceptanceHost
{
    ...
    public static async Task<WebApplication> StartLiveAnalysisAsync(
        string database, string url, ServiceAcceptanceBudget budget)
    {
        ...
        WebApplication app = ResearcherAnalysisService.Program.CreateApplication(
            ["--environment", "Testing", "--urls", url], builder =>
            {
                ...
                builder.Services.AddTransient<ServiceAcceptanceBudgetHandler>();
                builder.Services.AddHttpClient<GeminiArticleClient>()
                    .ConfigurePrimaryHttpMessageHandler(() => new HttpClientHandler { AllowAutoRedirect = false })
                    .AddHttpMessageHandler<ServiceAcceptanceBudgetHandler>();
        ...
        string database, string analysisUrl, string mode) => new()
    {
        ...
        ["FacultyAssistant:PollSeconds"] = "1", ["FacultyAssistant:RequestTimeoutSeconds"] = "330",
    ...
    : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    {
        if (!Request.Headers.TryGetValue(ServiceAcceptanceHost.Header, out var value) ||
            value != ServiceAcceptanceHost.HeaderValue) return Task.FromResult(AuthenticateResult.NoResult());
...
internal sealed class AcceptanceAccessService : IAcademicProductAccessService
{
        ...
        AcademicProductAccessRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.SubjectPersonelId != ServiceAcceptanceHost.SubjectId) throw new AcademicProductAccessDeniedException();
        return Task.FromResult(new AcademicProductAccessGrant("service-acceptance-grant",
            "service-acceptance-actor", ServiceAcceptanceHost.SubjectId, request.Operation));
    }
    ...
    public Task<AcademicProductAccessGrant> ReauthorizeAsync(
        ...
        persistedGrant.SubjectPersonelId == ServiceAcceptanceHost.SubjectId
